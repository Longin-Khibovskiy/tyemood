...
if (isset($_POST['submit_feedback'])) {
    ...
    } elseif (strlen($cleanPhone) < 9 || strlen($cleanPhone) > 11) {
        echo "<script>alert('Номер телефона должен содержать от 9 до 11 цифр.');</script>";
    } else {

        $stmt = $link->prepare("INSERT INTO feedback (`name`, phone_number) VALUES (?, ?)");
        $stmt->bind_param("ss", $name, $phone);
        if ($stmt->execute()) {
            echo "<script>alert('Спасибо! Мы свяжемся с вами в ближайшее время.');</script>";
        } else {
                ...
                if ($row['name'] != 'Лонгсливы'):
                    ...
                    <div class="category_container" style="background-image: url(<?= $row['image'] ?>);">
                        ...
                        <div class="category_svg_container">
                            <svg xmlns="http://www.w3.org/2000/svg" onclick="location.href = '<?= $row['link'] ?>'"
    ...
    <div class="contacts_container" id="contacts">
        ...
        <div class="contact_container">
            ...
            <form class="contact_right_container" method="POST" action="/">
                ...
                <input type="text" name="fullname" placeholder="Фамилия и Имя" class="regular_03" required>
                <input type="text" name="phone" placeholder="+7 999 999-99-99" class="regular_03" required>
                <button class="button_18" type="submit" name="submit_feedback">Получить консультацию</button>
            </form>
        </div>
    </div>
</section>